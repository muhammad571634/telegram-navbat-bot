...
TOKEN = "8168421097:AAFv9HBpeIY7Y0I8E1rr62psRFtzXbNZm-M"
...
await update.message.reply_text("Xabar matni")
        f"✅ Navbatga qo‘shildingiz!\n"
        f"👤 Ism: {name}\n"
        f"📞 Telefon: {phone}\n"
        f"🔢 Raqamingiz: {len(queue)}"
    )
async def if update.effective_user.id != ADMIN_ID:
    await update.message.reply_text("❌ Siz admin emassiz")
    return
list_queue(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not queue:
...
async def if update.effective_user.id != ADMIN_ID:
    await update.message.reply_text("❌ Siz admin emassiz")
    return
next_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not queue:
        await update.message.reply_text("📭 Navbat bo‘sh")
        return
